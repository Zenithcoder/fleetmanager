...
class DriversApi extends Controller {
		...
		if ($bookings->toArray() == null) {
			...
			foreach ($bookings as $book) {
				if ($book->booking_type == 0) {
					...
					if (strtotime($book->journey_date . " " . $book->journey_time) >= strtotime("-2 minutes")) {
	...
	public function accept_ride_request(Request $request) {
		...
		if ($booking == null) {
			...
			if ($u->getMeta('vehicle_id') != null) {
				...
				$booking->save();
				$user_details = array('user_id' => $booking->customer_id, 'user_name' => $booking->customer->name, 'mobno' => $booking->customer->getMeta('mobno'), 'profile_pic' => $booking->customer->getMeta('profile_pic'));
				$this->accept_ride_notification($booking->id);
				$data['success'] = 1;
	...
	public function accept_ride_notification($id) {
	...
	public function single_ride_info(Request $request) {
		...
		if ($booking == null) {
			...
			$data['data'] = array('rideinfo' => $ride_info, 'user_details' => $user_details, 'ride_review' => $ride_reviews,
				'fare_breakdown' => array('base_fare' => Hyvikk::fare(strtolower(str_replace(' ', '', $booking->vehicle->types->vehicletype)) . '_base_fare'), 'ride_amount' => $booking->getMeta('total'), 'extra_charges' => 0, 'payment_mode' => 'CASH')); //done
	...
	public function destination_reached(Request $request) {
		...
		if ($booking == null) {
			...
			$data['data'] = array('rideinfo' => $rideinfo, 'user_details' => $user_details, 'ride_review' => $reviews, 'fare_breakdown' => array('base_fare' => Hyvikk::fare(strtolower(str_replace(' ', '', $booking->vehicle->types->vehicletype)) . '_base_fare'), 'ride_amount' => $booking->getMeta('total'), 'extra_charges' => 0)); //done
		}
		return $data;
	}
