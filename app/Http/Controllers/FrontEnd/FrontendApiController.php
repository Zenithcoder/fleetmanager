...
class FrontendApiController extends Controller {
	...
	public function vehicles() {
		...
		foreach ($vehicles as $v) {
			...
			$data[] = array('id' => "$v->id", 'make' => $v->make, 'model' => $v->model, 'year' => $v->year, 'lic_plate' => $v->license_plate, 'vehicle_type' => $v->types->displayname, 'vehicle_image' => $url, 'average' => $v->average);
	...
	public function booking_history($id) {
		...
		foreach ($bookings as $booking) {
			...
			$data[] = array(
				'journey_date' => date('Y-m-d', strtotime($booking->journey_date)),
